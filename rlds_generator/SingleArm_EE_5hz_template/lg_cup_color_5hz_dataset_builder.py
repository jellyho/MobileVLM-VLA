...
class LgStackBowls5hzDataset(tfds.core.GeneratorBasedBuilder):
    ...
    def _generate_examples(self, path) -> Iterator[Tuple[str, Any]]:
        ...
        def _parse_example(hdf5_path):
            ...
            delta_ee_right = ee_pos[4:, :6] - ee_pos[:-4, :6]
            grp_right = ee_pos[4:, 6]

            # assemble episode --> here we're assuming demos so we set reward to 1 at the end
            episode = []
            for i in range(length - 4):
                language_embedding = self._embed([nli[i]])[0].numpy()
                delta_ee = np.concatenate([delta_ee_right, np.array(grp_right[i, -1]).reshape(1)])
                episode.append({
                        'observation': {
                            'image': image[i],
                        },
