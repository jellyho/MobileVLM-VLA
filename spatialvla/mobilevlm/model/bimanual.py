...
class TwinVLA(SpatialVLAForCausalLM):
    ...
    def _make_causal_mask(self, input_ids_shape: torch.Size, dtype: torch.dtype, device: torch.device, past_key_values_length: int = 0):
        """
    ...
    def joint_forward(self, 
        ...
        indep_atm = self.prepare_decoder_attention_mask(
            indep_atm, (batch_size, seq_length), inputs_lr[0][3], past_key_values_length
        )
        joint_atm = self.prepare_decoder_attention_mask(
            joint_atm, (batch_size, seq_length), inputs_lr[0][3], past_key_values_length
        )
        # indep_atm = self.model_r._update_causal_mask(indep_atm, inputs_lr[0][3])
        # joint_atm = self.model_r._update_causal_mask(joint_atm, inputs_lr[0][3])
