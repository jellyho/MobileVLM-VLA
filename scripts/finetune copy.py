...
optimizer_grouped_parameters = [
    ...
    {
        "params": [
            p for n, p in model.named_parameters() if (n not in decay_parameters and n not in unused_parameters and p.requires_grad)
        ],
        "weight_decay": 0.0,
        "lr": training_args.learning_rate,
        "eps":training_args.adam_epsilon
...
elif training_args.lr_schedule_type == 'cosine':
    warmup_steps = int(training_args.max_steps * training_args.warmup_ratio)
...
with tqdm(total=training_args.max_steps, leave=False) as progress:
    ...
    for batch_idx, batch in enumerate(dataloader):
        with torch.autocast('cuda', dtype=dtype):
            if model_args.head_args['head_type'] in ['Diffusion', 'DiffusionPolicy']:
