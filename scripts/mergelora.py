...
from spatialvla.mobilevlm.model.bimanual import TwinVLA, TwinVLAConfig
...
def merge_lora_twinvla(single_model_path, model_path, save_path):
    # kwargs = {"device_map":"auto", "torch_dtype": torch.bfloat16}
    kwargs = {"torch_dtype": torch.bfloat16}
    lora_cfg_pretrained = TwinVLAConfig.from_pretrained(single_model_path)
    ...
    # Loading weight from base model
    model = TwinVLA.from_pretrained(single_model_path, config=lora_cfg_pretrained, **kwargs)
    print("🔜 Don't worry, it will load additional weight soon later...")
    token_num, tokem_dim = model.lm_head.out_features, model.lm_head.in_features
    if model.lm_head.weight.shape[0] != token_num:
        model.lm_head.weight = torch.nn.Parameter(torch.empty(token_num, tokem_dim, device=model.device, dtype=model.dtype))
