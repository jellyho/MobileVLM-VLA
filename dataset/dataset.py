...
from dataset import obs_transforms, traj_transforms
from dataset.utils import goal_relabeling, task_augmentation
from dataset.utils.data_utils import (
    allocate_threads,
...
import copy
tf.config.run_functions_eagerly(True) ## Make True for dataloader
...
def make_single_dataset(
    ...
    *,
    ...
    dataset = apply_trajectory_transforms(dataset.repeat(), **traj_transform_kwargs, train=train).flatten()
    print()
...
class RLDSBatchTransform:
    ...
    def __call__(self, rlds_batch: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return dict(pixel_values=new_img, input_ids=input_ids, action=action)
...
class RLDSDataset(IterableDataset):
    def __init__(
        ...
        *,
        ...
        # Not supporting mixed dataset so far. Assuming single dataset
...
def numpy_to_json_compatible(data):
    """Converts numpy arrays in a dictionary to lists for JSON compatibility."""
    if isinstance(data, dict):
        return {k: numpy_to_json_compatible(v) for k, v in data.items()}
    elif isinstance(data, np.ndarray):
        return data.tolist()  # Convert numpy arrays to lists
    else:
        return data

def save_statistics_to_json(data, filename):
    """Saves a dictionary to a JSON file, handling numpy arrays."""
    json_compatible_data = numpy_to_json_compatible(data)
